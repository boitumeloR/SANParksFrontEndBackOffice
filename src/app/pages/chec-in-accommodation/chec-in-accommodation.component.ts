...
export class ChecInAccommodationComponent implements OnInit {
  ...
  SearchBooking() {
    ...
    this.checkServ.SearchAccommodationBooking(this.global.GetServer(), searchObj).subscribe(res => {
      if (res.Session !== null) {
        if (res.Found === true) {
  ...
  Submit() {
    ...
    this.checkServ.AccommodationCheckIn(this.global.GetServer(), check)
    .subscribe(checked => {
      if (checked.Session == null) {
        this.router.navigateByUrl('Login');
        const err = this.dialog.open(ErrorModalComponent, {
        ...
        if (checked.Success === true) {
          ...
          const err = this.dialog.open(SuccessModalComponent, {
            disableClose: true,
            data: {successMessage: `Successfully Checked in. Welcome the client to their respective units.`}
          });
        } else {
          const err = this.dialog.open(ErrorModalComponent, {
            disableClose: true,
