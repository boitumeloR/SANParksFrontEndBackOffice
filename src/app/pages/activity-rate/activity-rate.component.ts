...
export class ActivityRateComponent implements OnInit {
  ...
  addRate() {
    const dialogRef = this.dialog.open(AddActivityRateComponent, {disableClose: true});
  ...
  updateRate(rate) {
    const dialogRef = this.dialog.open(UpdateActivityRateComponent);
  }

  deleteRate(rate) {
    const dialogRef = this.dialog.open(DeleteActivityRateComponent);
  }
